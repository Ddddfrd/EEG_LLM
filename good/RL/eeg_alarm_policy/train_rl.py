"""Run R3 controls, tabular Q-learning, and five-seed PPO on cached scores."""

from __future__ import annotations

import argparse
import hashlib
import json
import os
import platform
import uuid
from pathlib import Path
from typing import Any

import numpy as np
import torch

from .artifacts import load_prediction_artifact, save_content_addressed_json
from .cohort import SelectionObjective, evaluate_cohort
from .environment import AlarmRewardConfig, rollout_policy
from .features import compute_enrollment_statistics
from .rl_training import (
    PPOConfig,
    TabularConfig,
    actor_actions,
    discretize_observation,
    train_ppo,
    train_tabular_q,
)
from .rules import TemporalRule, apply_rule, enrollment_probabilities

SCHEMA_VERSION = "eeg_rl_phase_r3_v1"
PPO_SEEDS = (11, 22, 33, 44, 55)


def _load_subject(directory: Path, subject: str):
    matches = sorted(directory.glob(f"predictions_{subject}_*.json"))
    if len(matches) != 1:
        raise FileNotFoundError(f"Expected one prediction artifact for {subject}")
    return load_prediction_artifact(matches[0])


def _metrics(
    timeline,
    actions: np.ndarray,
    *,
    objective: SelectionObjective,
    refractory_seconds: float,
) -> dict[str, Any]:
    cohort = evaluate_cohort(
        {timeline.subject_id: timeline},
        {timeline.subject_id: actions},
        refractory_seconds=refractory_seconds,
    )
    pooled = cohort["pooled"]
    return {
        "j_score": objective.score(pooled),
        "guardrail_pass": objective.passes_guardrail(
            pooled,
            cohort["patient_summary"],
        ),
        "pooled": pooled,
        "patient_summary": cohort["patient_summary"],
    }


def _save_torch_checkpoint(
    model: torch.nn.Module,
    destination_dir: Path,
    *,
    seed: int,
    config: PPOConfig,
) -> tuple[Path, str]:
    destination_dir.mkdir(parents=True, exist_ok=True)
    temporary = destination_dir / f".ppo_seed_{seed}.{uuid.uuid4().hex}.tmp"
    torch.save(
        {
            "schema_version": SCHEMA_VERSION,
            "seed": seed,
            "config": config.to_dict(),
            "state_dict": model.state_dict(),
        },
        temporary,
    )
    digest = hashlib.sha256(temporary.read_bytes()).hexdigest()
    destination = destination_dir / f"ppo_seed_{seed}_{digest[:12]}.pt"
    if destination.exists():
        temporary.unlink()
    else:
        os.replace(temporary, destination)
    return destination, digest


def _save_actions(
    actions: np.ndarray,
    output_dir: Path,
    *,
    name: str,
) -> tuple[Path, str]:
    values = np.asarray(actions, dtype=np.uint8)
    digest = hashlib.sha256(values.tobytes()).hexdigest()
    output_dir.mkdir(parents=True, exist_ok=True)
    destination = output_dir / f"{name}_{digest[:12]}.npy"
    if not destination.exists():
        temporary = output_dir / f".{destination.name}.{uuid.uuid4().hex}.tmp"
        with temporary.open("wb") as stream:
            np.save(stream, values, allow_pickle=False)
        os.replace(temporary, destination)
    return destination, digest


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--artifact-dir", type=Path, required=True)
    parser.add_argument("--robust-rule-result", type=Path, required=True)
    parser.add_argument("--supervised-result", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--ppo-epochs", type=int, default=20)
    args = parser.parse_args(argv)

    training_artifact = _load_subject(args.artifact_dir, "chb20")
    selection_artifact = _load_subject(args.artifact_dir, "chb21")
    training_timeline = training_artifact.timeline
    selection_timeline = selection_artifact.timeline
    training_enrollment = compute_enrollment_statistics(
        enrollment_probabilities(training_timeline)
    )
    selection_enrollment = compute_enrollment_statistics(
        enrollment_probabilities(selection_timeline)
    )
    reward_config = AlarmRewardConfig()
    objective = SelectionObjective(
        lambda_fa=0.02,
        lambda_latency=0.001,
        minimum_event_sensitivity=0.8,
        minimum_patient_event_sensitivity=0.8,
    )

    controls: dict[str, Any] = {}
    always_actions = np.ones(selection_timeline.row_count, dtype=np.uint8)
    never_actions = np.zeros(selection_timeline.row_count, dtype=np.uint8)
    controls["always_alarm"] = _metrics(
        selection_timeline,
        always_actions,
        objective=objective,
        refractory_seconds=reward_config.refractory_seconds,
    )
    controls["never_alarm"] = _metrics(
        selection_timeline,
        never_actions,
        objective=objective,
        refractory_seconds=reward_config.refractory_seconds,
    )
    random_results = []
    for seed in PPO_SEEDS:
        rng = np.random.default_rng(seed)
        actions = rng.integers(0, 2, size=selection_timeline.row_count, dtype=np.uint8)
        random_results.append(
            {
                "seed": seed,
                **_metrics(
                    selection_timeline,
                    actions,
                    objective=objective,
                    refractory_seconds=reward_config.refractory_seconds,
                ),
            }
        )
    controls["random"] = random_results

    tabular_config = TabularConfig()
    q_values = train_tabular_q(
        training_timeline,
        training_enrollment,
        reward_config=reward_config,
        config=tabular_config,
        seed=42,
    )
    tabular_actions, tabular_rewards = rollout_policy(
        selection_timeline,
        selection_enrollment,
        lambda observation: int(np.argmax(q_values[discretize_observation(observation)])),
        reward_config=reward_config,
    )
    tabular_action_path, tabular_action_hash = _save_actions(
        tabular_actions,
        args.output_dir / "actions",
        name="tabular_chb21",
    )
    controls["tabular_q"] = {
        **_metrics(
            selection_timeline,
            tabular_actions,
            objective=objective,
            refractory_seconds=reward_config.refractory_seconds,
        ),
        "reward_components": tabular_rewards,
        "action_path": str(tabular_action_path),
        "action_sha256": tabular_action_hash,
    }

    ppo_config = PPOConfig(epochs=args.ppo_epochs)
    ppo_results: list[dict[str, Any]] = []
    for seed in PPO_SEEDS:
        model, history = train_ppo(
            training_timeline,
            training_enrollment,
            reward_config=reward_config,
            config=ppo_config,
            seed=seed,
        )
        actions, rewards = actor_actions(
            model,
            selection_timeline,
            selection_enrollment,
            reward_config=reward_config,
        )
        checkpoint, checkpoint_hash = _save_torch_checkpoint(
            model,
            args.output_dir / "checkpoints",
            seed=seed,
            config=ppo_config,
        )
        action_path, action_hash = _save_actions(
            actions,
            args.output_dir / "actions",
            name=f"ppo_seed_{seed}_chb21",
        )
        ppo_results.append(
            {
                "seed": seed,
                **_metrics(
                    selection_timeline,
                    actions,
                    objective=objective,
                    refractory_seconds=reward_config.refractory_seconds,
                ),
                "reward_components": rewards,
                "history": history,
                "checkpoint": str(checkpoint),
                "checkpoint_sha256": checkpoint_hash,
                "action_path": str(action_path),
                "action_sha256": action_hash,
            }
        )
        print(
            f"ppo seed {seed}: J={ppo_results[-1]['j_score']} "
            f"guardrail={ppo_results[-1]['guardrail_pass']}",
            flush=True,
        )

    finite_ppo = sorted(
        (result for result in ppo_results if result["j_score"] is not None),
        key=lambda result: result["j_score"],
    )
    median_ppo = finite_ppo[len(finite_ppo) // 2] if finite_ppo else None

    robust_payload = json.loads(args.robust_rule_result.read_text(encoding="utf-8"))
    robust_rule = TemporalRule(**robust_payload["selected"]["rule"])
    robust_actions = apply_rule(selection_timeline, robust_rule)
    robust_metrics = _metrics(
        selection_timeline,
        robust_actions,
        objective=objective,
        refractory_seconds=robust_rule.refractory_seconds,
    )
    supervised_payload = json.loads(args.supervised_result.read_text(encoding="utf-8"))
    supervised_candidates = {
        name: result["selected"]
        for name, result in supervised_payload["results"].items()
        if result["selected"] is not None
    }
    best_supervised_name, best_supervised = max(
        supervised_candidates.items(),
        key=lambda item: item[1]["j_score"],
    )
    comparison_score = max(
        float(robust_metrics["j_score"]),
        float(best_supervised["j_score"]),
    )
    ppo_promoted = bool(
        median_ppo
        and median_ppo["guardrail_pass"]
        and float(median_ppo["j_score"]) > comparison_score
    )
    payload: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "method": "record-level cached-probability RL development",
        "training_subject": "chb20",
        "selection_subject": "chb21",
        "training_artifact_id": training_artifact.metadata["artifact_id"],
        "selection_artifact_id": selection_artifact.metadata["artifact_id"],
        "reward_config": vars(reward_config),
        "objective": {
            "lambda_fa": objective.lambda_fa,
            "lambda_latency": objective.lambda_latency,
            "minimum_event_sensitivity": objective.minimum_event_sensitivity,
            "minimum_patient_event_sensitivity": (
                objective.minimum_patient_event_sensitivity
            ),
        },
        "controls": controls,
        "ppo": {
            "config": ppo_config.to_dict(),
            "seeds": list(PPO_SEEDS),
            "results": ppo_results,
            "selection_rule": "median validation J seed",
            "selected": median_ppo,
            "promoted": ppo_promoted,
        },
        "development_comparators": {
            "robust_fixed_rule": {
                "rule": robust_rule.to_dict(),
                **robust_metrics,
            },
            "best_supervised_name": best_supervised_name,
            "best_supervised": best_supervised,
        },
        "runtime": {
            "python": platform.python_version(),
            "numpy": np.__version__,
            "torch": torch.__version__,
        },
    }
    result_path = save_content_addressed_json(
        payload,
        args.output_dir,
        hash_field="result_sha256",
        stem="phase_r3",
    )
    print(json.dumps({"result": str(result_path), "ppo_promoted": ppo_promoted}, indent=2))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
